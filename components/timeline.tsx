      ...
      {timelineData.map((item, i) => (
        <div key={i} className="relative flex gap-6">
          ...
          <div className="flex flex-col items-center">
            <div
              className={`w-3 h-3 rounded-full ${item.current ? "bg-gray-900 dark:bg-[#ffffff]" : "bg-gray-400 dark:bg-[#4d4d4d]"}`}
            />
          ...
          <div className="flex-1 pb-8">
            <div className="flex items-center gap-2 mb-1">
              ...
              {item.current && (
                <span className="text-xs bg-gray-900 text-white dark:bg-[#ffffff] dark:text-[#000000] px-2 py-0.5 rounded">
                  {t("current")}
                </span>
              )}
            </div>
